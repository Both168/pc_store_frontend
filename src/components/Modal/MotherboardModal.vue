      ...
      <div v-if="modelValue" class="modal-overlay" @click.self="close">
        <div class="modal-container neumorphic-card">
          <div class="modal-header">
            <h2 class="modal-title">Add Motherboard</h2>
          ...
          <div class="modal-body">
            <MotherboardFilter v-model="filters" />
            <div v-if="isLoading" class="modal-cards-loading">Loading motherboards...</div>
            <template v-else>
              <div class="modal-cards-grid">
                <LaptopCard
                  ...
                  :detail-path="'/component/motherboard'"
                  :index="index"
...
import MotherboardFilter from '../pc_part_filter/MotherboardFilter.vue'
import LaptopCard from '../common/LaptopCard.vue'
import crossCircleIcon from '../../assets/icons/setup/cross-circle.png'
import Pagination from '../common/Pagination.vue'
import { fetchMotherboardCards, type MotherboardCard } from '../../services/apiComponent'
...
const filters = ref({
  min_price: null as number | null, max_price: null as number | null,
  form_factor: null as number | null, socket: null as number | null, ram_type: null as string | null, wifi: null as number | null,
  brand: null as number | null, sort: null as 'price_asc' | 'price_desc' | 'name_asc' | 'name_desc' | null,
})
const cards = ref<MotherboardCard[]>([])
...
async function load() {
  ...
  try {
    ...
    if (filters.value.form_factor != null) f.form_factor_id = filters.value.form_factor
    if (filters.value.socket != null) f.socket = filters.value.socket
    if (filters.value.ram_type != null) f.ram_type = filters.value.ram_type
    if (filters.value.wifi != null) f.wifi_id = filters.value.wifi
    if (filters.value.brand != null) f.brand_id = filters.value.brand
    if (filters.value.sort) f.sort = filters.value.sort
    const result = await fetchMotherboardCards(f)
