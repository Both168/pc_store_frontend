      ...
      <div v-if="modelValue" class="modal-overlay" @click.self="close">
        <div class="modal-container neumorphic-card">
          <div class="modal-header">
            <h2 class="modal-title">Add Power Supply</h2>
          ...
          <div class="modal-body">
            <PsuFilter v-model="filters" />
            <div v-if="isLoading" class="modal-cards-loading">Loading power supplies...</div>
            <template v-else>
...
const filters = ref({ min_price: null as number | null, max_price: null as number | null, brand: null as number | null, watt: null as number | null, plus80: null as string | null })
const cards = ref<PsuCard[]>([])
...
async function load() {
  ...
  try {
    ...
    if (filters.value.watt != null) f.watt_id = filters.value.watt
    if (filters.value.plus80) f.plus80 = filters.value.plus80
    const result = await fetchPsuCards(f)
