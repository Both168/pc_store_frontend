      ...
      <div v-if="modelValue" class="modal-overlay" @click.self="close">
        <div class="modal-container neumorphic-card">
          <div class="modal-header">
            <h2 class="modal-title">Add CPU Cooler</h2>
          ...
          <div class="modal-body">
            <CoolerFilter v-model="filters" />
            <div v-if="isLoading" class="modal-cards-loading">Loading coolers...</div>
            <template v-else>
...
import CoolerFilter from '../pc_part_filter/CoolerFilter.vue'
import LaptopCard from '../common/LaptopCard.vue'
import crossCircleIcon from '../../assets/icons/setup/cross-circle.png'
import Pagination from '../common/Pagination.vue'
import { fetchCoolerCards, type CoolerCard } from '../../services/apiComponent'
...
const filters = ref({ min_price: null as number | null, max_price: null as number | null, brand: null as number | null, type: null as number | null })
const cards = ref<CoolerCard[]>([])
...
async function load() {
  ...
  try {
    ...
    if (filters.value.type != null) f.type_id = filters.value.type
    const result = await fetchCoolerCards(f)
