      ...
      <div v-if="modelValue" class="modal-overlay" @click.self="close">
        <div class="modal-container neumorphic-card">
          <div class="modal-header">
            <h2 class="modal-title">Add GPU</h2>
          ...
          <div class="modal-body">
            <GpuFilter v-model="gpuFilters" />
            <div v-if="isLoading" class="modal-cards-loading">Loading GPUs...</div>
            <template v-else>
...
const gpuFilters = ref({ min_price: null as number | null, max_price: null as number | null, type: null as string | null, brand: null as number | null })
const cards = ref<GpuCard[]>([])
...
async function load() {
  ...
  try {
    const f: { min_price?: number; max_price?: number; type?: string; brand_id?: number; page?: number } = { page: pagination.value.current_page }
    if (gpuFilters.value.min_price != null) f.min_price = gpuFilters.value.min_price
    if (gpuFilters.value.max_price != null) f.max_price = gpuFilters.value.max_price
    if (gpuFilters.value.type) f.type = gpuFilters.value.type
    if (gpuFilters.value.brand != null) f.brand_id = gpuFilters.value.brand
    const result = await fetchGpuCards(f)
...
watch(() => gpuFilters.value, () => { pagination.value.current_page = 1; load() }, { deep: true })
